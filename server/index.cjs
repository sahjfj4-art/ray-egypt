...
// Root route: provide a friendly message so browser GET / doesn't show "Cannot GET /"
app.get('/', (req, res) => {
  const vitePort = 3000;
  res.send(`
    <html>
      <head><meta charset="utf-8"><title>Ray Gemini Proxy</title></head>
      <body style="font-family: Arial, Helvetica, sans-serif; padding:24px;">
        <h2>Ray Gemini Proxy</h2>
        <p>This server only exposes a POST endpoint at <code>/api/gemini</code> for the frontend.</p>
        <p>To view the frontend app, open the Vite dev server at <a href="http://localhost:${vitePort}" target="_blank">http://localhost:${vitePort}</a>.</p>
        <p>If you expected this server to serve the frontend, configure a reverse proxy or serve static files from the frontend's <code>dist</code> directory after building.</p>
      </body>
    </html>
  `);
});
