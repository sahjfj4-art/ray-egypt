...
import { 
  ...
  Calendar, Settings, LogOut, LayoutDashboard, Clock, Clipboard
} from 'lucide-react';
...
const HardwareDashboard: React.FC<Props> = ({ onLogout, onSwitchType }) => {
  const [activeTab, setActiveTab] = useState('overview');
  const config = dashboardConfigs['hardware'];
  const theme = colorClasses['gray'];
  ...
  const Sidebar = () => (
    <aside className="w-64 bg-gray-900 dark:bg-black text-white hidden md:flex flex-col shadow-xl z-30 transition-colors">
      <div className="p-6 border-b border-gray-800 flex items-center gap-3">
         ...
         <div>
           <h1 className="font-bold text-xl tracking-wide">الأدوات والمعدات المتخصصة</h1>
           <p className="text-gray-400 text-[10px]">للأدوات والمعدات والإصلاح</p>
         </div>
      </div>

      <nav className="flex-1 p-4 space-y-2 overflow-y-auto">
        <SidebarItem icon={LayoutDashboard} label="لوحة التحكم" id="overview" active={activeTab} setTab={setActiveTab} />
        <SidebarItem icon={Package} label="الأدوات والمعدات" id="products" active={activeTab} setTab={setActiveTab} highlight />
        ...
        <SidebarItem icon={Clock} label="التأجير" id="rentals" active={activeTab} setTab={setActiveTab} />
        <SidebarItem icon={Wrench} label="الصيانة" id="repairs" active={activeTab} setTab={setActiveTab} />
        <SidebarItem icon={Settings} label="خدمات" id="services" active={activeTab} setTab={setActiveTab} />
  ...
  const renderContent = () => {
    switch (activeTab) {
      case 'products':
        return <ProductsManager businessType="hardware" />;
      case 'inventory':
      ...
      case 'rentals':
        return <UniversalDataView type="rentals" theme={theme} />;
      case 'repairs':
        return <UniversalDataView type="repairs" theme={theme} />;
      case 'services':
      ...
      case 'overview':
      default:
        return <HardwareOverview setActiveTab={setActiveTab} />;
    }
  };

  return (
    <div className="min-h-screen bg-gray-50/30 dark:bg-gray-900 flex font-sans transition-colors">
      <Sidebar />
      <main className="flex-1 overflow-y-auto h-screen flex flex-col">
        <Header 
          config={config} 
          currentBusinessType="hardware" 
...
const SidebarItem = ({ icon: Icon, label, id, active, setTab, highlight }: any) => (
  <button 
    ...
    className={`w-full flex items-center gap-3 p-3 rounded-xl transition-all duration-200 
      ...
      ${highlight && active !== id ? 'text-gray-400' : ''}
    `}
  >
